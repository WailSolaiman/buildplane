...
export function Nav() {
  ...
  return (
    <>
      ...
      >
        <nav
          className="relative z-10 mx-auto flex max-w-7xl flex-row items-center justify-between gap-4 px-6 py-3 sm:px-8 sm:py-3.5"
          aria-label="Primary"
        >
          <NavLink
            to="/"
            className="font-display shrink-0 text-xl tracking-tight text-[hsl(var(--foreground))] sm:text-2xl"
            onClick={() => setMenuOpen(false)}
          >
            Buildplane
          ...
          <div className="flex shrink-0 items-center gap-2 sm:gap-3">
            ...
            <button
              type="button"
              className="flex size-9 items-center justify-center rounded-full border border-[hsl(var(--border)/0.45)] text-[hsl(var(--foreground))] transition-colors hover:bg-[hsl(var(--foreground)/0.06)] md:hidden"
        ...
        <div
          ...
          aria-label="Mobile"
        >
          <div className="flex flex-col px-6 py-4 sm:px-8">
            {links.map(({ to, label, end: endMatch }) => (
              <NavLink
                key={to}
                to={to}
                ...
                className={({ isActive }) =>
                  [
                    'border-b border-[hsl(var(--border)/0.25)] py-3 text-[0.9375rem] transition-colors last:border-b-0',
                    isActive
                      ? 'text-[hsl(var(--foreground))]'
